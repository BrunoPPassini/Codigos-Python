...
num = [1, 0]
den = [1]
s = tf(num, den) # Auxiliar para encontrar velocidade

###### Dando step e criando vetor de tempo ####
n_cont = round(time_final / 0.03) + 1
t_cont = linspace(0, time_final, n_cont)
y_cont, t_c = step(MalhaFechadaC*s, t_cont) # Step para velocidade
n_points = round(time_final / Ts) + 1
time = linspace(0, time_final, n_points)
plt.plot(t_cont,y_cont*0.14, label = 'Malha Fechada', color = ('blue'), linewidth = 2) # Construção do gráfico contínuo e achando tempo
